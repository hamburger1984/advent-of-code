...
pub fn process_part1(input: &str) -> String {
    let (width, height, map, start, goal) = read_map(input);

    let path = a_star(start, goal, width, height, part1_heuristic, |x, y| {
        ...
        if y > 0 && part1_reachable(current_level, map[to_index_xy(x, y - 1, width)]) {
            neighbors.push((x, y - 1));
        }

        // bottom neighbor present and reachable?
        if y < height - 1 && part1_reachable(current_level, map[to_index_xy(x, y + 1, width)]) {
            neighbors.push((x, y + 1));
        }

        // left neighbor present and reachable?
        if x > 0 && part1_reachable(current_level, map[to_index_xy(x - 1, y, width)]) {
            neighbors.push((x - 1, y));
        }

        // right neighbor present and reachable?
        if x < width - 1 && part1_reachable(current_level, map[to_index_xy(x + 1, y, width)]) {
            neighbors.push((x + 1, y));
        }
...
fn part1_heuristic(x1: usize, y1: usize, x2: usize, y2: usize) -> usize {
    // Use the Manhattan distance as the heuristic
    (x1.abs_diff(x2) + y1.abs_diff(y2)).pow(2)
}

fn part1_reachable(current_level: u8, next_level: u8) -> bool {
    return current_level >= next_level || current_level as i16 == next_level as i16 - 1;
}

pub fn process_part2(input: &str) -> String {
    let (width, height, map, _start, goal) = read_map(input);

    let path = a_star_2(
        goal,
        |x, y| part2_is_goal(x, y, &map, width),
        width,
        height,
        |x, y| part2_heuristic(x, y, &map, width),
        |x, y| {
            // Return the four neighboring cells
            let mut neighbors = Vec::new();
            ...
            // top neighbor present and reachable?
            if y > 0 && part2_reachable(current_level, map[to_index_xy(x, y - 1, width)]) {
                neighbors.push((x, y - 1));
            }

            // bottom neighbor present and reachable?
            if y < height - 1 && part2_reachable(current_level, map[to_index_xy(x, y + 1, width)]) {
                neighbors.push((x, y + 1));
            }

            // left neighbor present and reachable?
            if x > 0 && part2_reachable(current_level, map[to_index_xy(x - 1, y, width)]) {
                neighbors.push((x - 1, y));
            }

            // right neighbor present and reachable?
            if x < width - 1 && part2_reachable(current_level, map[to_index_xy(x + 1, y, width)]) {
                neighbors.push((x + 1, y));
            }
...
fn part2_is_goal(x: usize, y: usize, map: &Vec<u8>, width: usize) -> bool {
    let level = map[to_index_xy(x, y, width)];
    return level == 0;
}

fn part2_reachable(current_level: u8, next_level: u8) -> bool {
    return next_level >= current_level || next_level as i16 == current_level as i16 - 1;
}

fn part2_heuristic(x: usize, y: usize, map: &Vec<u8>, width: usize) -> usize {
    // Use current height as heuristic - we want to get to zero as fast as possible
    return (map[to_index_xy(x, y, width)] as usize).pow(2);
}

fn read_map(input: &str) -> (usize, usize, Vec<u8>, (usize, usize), (usize, usize)) {
...
fn a_star_2(
    ...
    width: usize,
    height: usize,
    mut heuristic: impl FnMut(usize, usize) -> usize,
    mut successors: impl FnMut(usize, usize) -> Vec<(usize, usize)>,
) -> Option<Vec<(usize, usize)>> {
    let start_index = to_index_pos(&start, width);
    ...
    while let Some(State { index, cost, .. }) = frontier.pop() {
        ...
        // Check if we have reached the goal
        if is_goal(cell_position.0, cell_position.1) {
            let mut map_dump = vec!['.'; width * height];
