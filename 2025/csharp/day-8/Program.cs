...
static class Solution
{
    ...
    public static string Part2(string input)
    {
        ...
        var unconnected = Enumerable.Range(0, vectors.Count).ToHashSet();
        var connected = new List<(int,int)>();
        ...
        while(unconnected.Count > 0)
        {
            ...
            unconnected.Remove(min.Item1);
            unconnected.Remove(min.Item2);
        }

        var lastConnection = connected.Last();

        return (vectors[lastConnection.Item1].Item1 * vectors[lastConnection.Item2].Item1).ToString();
    }
}
