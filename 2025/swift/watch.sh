...
echo "🔍 Swift TDD Watch Mode for $DAY_DIR"
...
if command -v fswatch &> /dev/null; then
    ...
    fswatch -o "$DAY_DIR/Sources" "$DAY_DIR/Tests" | while read f; do
        clear
        echo "🔄 Running tests for $DAY_DIR..."
        echo ""
        cd "$DAY_DIR" && swift test
        echo ""
    ...
    while true; do
        ...
        echo ""
        echo "✨ Waiting for changes..."
        inotifywait -qre modify,create,delete "$DAY_DIR/Sources" "$DAY_DIR/Tests"
    done
else
