...
# Update Package.swift with correct name
sed -i '' "s/name: \"day\"/name: \"day-${DAY}\"/g" "$DAY_DIR/Package.swift"
sed -i '' "s/name: \"day\",/name: \"day-${DAY}\",/g" "$DAY_DIR/Package.swift"
sed -i '' "s/dependencies: \\[\"day\"\\]/dependencies: [\"day-${DAY}\"]/g" "$DAY_DIR/Package.swift"
sed -i '' "s/@testable import day/@testable import day_${DAY}/g" "$DAY_DIR/Tests/SolutionTests.swift"
    ...
    if [ -f "$DAY_DIR/task.txt" ]; then
        ...
        if [ -f "$PARSE_SCRIPT" ] && [ -f "$GEN_SCRIPT" ]; then
            ...
            if [ $? -eq 0 ] && [ -n "$EXAMPLES" ]; then
                "$GEN_SCRIPT" swift "$DAY_DIR" "$EXAMPLES" 2>/dev/null && echo "✓ Tests generated with example data"
            fi
        fi
    fi
fi
...
echo ""
echo "To run:"
echo "  cd $DAY_DIR && swift run"
echo ""
echo "To test:"
echo "  cd $DAY_DIR && swift test"
